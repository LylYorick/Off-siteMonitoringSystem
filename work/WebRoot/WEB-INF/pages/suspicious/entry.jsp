...
<body>
    ...
    $.subscribe('add', function(event,data) {
    	...
    	document.location.href="<%=request.getContextPath()%>/suspicious/suspicious_add.shtml";
    	});  
    $.subscribe('bankview', function(event,data) { 
    		...
    		var r = $("#gridtable").jqGrid('getRowData',s);     		 		
	  		window.location.href="<%=request.getContextPath()%>/suspicious/suspicious_bankview.shtml?id="+r.id;
	    });    	  
    ...
    $.subscribe('createxls', function(event,data) {
    	...
    	//alert("cname="+cname+" lineid="+lineid+" starttime="+starttime+" endtime="+endtime+" ccid="+ccid);
	  	window.location.href="<%=request.getContextPath()%>/suspicious/suspicious_downloadbank.shtml?ccid="+ccid+"&cname="+cname+"&starttime="+starttime+"&endtime="+endtime+"&lineid="+lineid;
  	});	
    </script>
	<div class="grid">
		<fieldset>
			...
			<br>
			<s:form namespace="/suspicious" action="suspicious_banklist"
				method="post">
			...
			<div align="center">
				<sj:submit formIds="suspicious_banklist" id="searchbutton"
					value="查  询" timeout="2500" button="true" indicator="indicator"
				...
				<img id="indicator"
					src="<%=request.getContextPath()%>/images/027.gif" alt="Loading..."
					style="display: none" />
			</div>
		...
		<sj:submit id="grid_add_colsbutton" value="添加可疑交易" onClickTopics="add"
			button="true" /></s:if>
		<sj:submit id="grid_bankview_colsbutton" value="线索信息查询"
			onClickTopics="bankview" button="true" />

		<sj:grid id="gridtable" caption="重点可疑交易报告列表" dataType="json"
			href="../suspicious/suspicious_banklist.shtml" pager="true"
			gridModel="gridModel" rowList="10,15,20" rowNum="20"
			rownumbers="true" viewrecords="true" multiselect="true"
			cssStyle="line-height:30px;" onSelectRowTopics="rowselect"
			height="400">
			<sj:gridColumn name="id" index="id" title="ID" sortable="false"
				width="50" hidden="true" />
			<sj:gridColumn name="BOrgInformation.bname"
				index="BOrgInformation.bname" title="金融机构名称" sortable="false"
				width="200" />
			<sj:gridColumn name="updatetime" index="updatetime" title="报送时间"
				sortable="false" width="130" />
			<sj:gridColumn name="lineid" index="lineid" title="线索编号"
				sortable="false" width="150" />
			<sj:gridColumn name="cname" index="cname" title="客户姓名"
				sortable="false" width="150" />
			<sj:gridColumn name="clamt" index="clamt" title="本币累计金额"
				sortable="false" width="150" />
			<sj:gridColumn name="cfamt" index="cfamt" title="外币累计金额"
				sortable="false" width="150" />
		</sj:grid>
	</div>
