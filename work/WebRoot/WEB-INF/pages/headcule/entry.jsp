...
<head>
	<title>反洗钱调查信息管理>>线索来源(总行或兄弟行)查询</title>
</head>
<body>
<script type="text/javascript">
    ...
    $.subscribe('errorState', function(event,data) { 
 	...
 	$.subscribe('add', function(event,data) {
   	var s; 
   	s = $("#gridtable").jqGrid('getGridParam','selarrrow');
   	document.location.href="<%=request.getContextPath()%>/headcule/headcule_add.shtml";
   	}); 
   	$.subscribe('modify', function(event,data) {
   	var s = $("#gridtable").jqGrid('getGridParam','selarrrow');
   	var r = $("#gridtable").jqGrid('getRowData',s);
   	document.location.href="<%=request.getContextPath()%>/headcule/headcule_modify.shtml?hosid="+r.hosid;
   	});
   	
   	$.subscribe('delete', function(event,data) {
   	var s = $("#gridtable").jqGrid('getGridParam','selarrrow');
   	var r = $("#gridtable").jqGrid('getRowData',s);
   	document.location.href="<%=request.getContextPath()%>/headcule/headcule_delete.shtml?hosid="+r.hosid;
   	});
    </script>     
	<div class="grid">
		<fieldset>
		...
		<br>
		<s:form namespace="/headcule" action="headcule_list" method="post">

			<s:bean name="java.util.HashMap" id="qTableLayout">
				<s:param name="tablecolspan" value="%{4}" />
			</s:bean>
			<s:textfield label="案件总编号" name="hid" id="hid"></s:textfield>
		...
		<div align="center">
			<sj:submit formIds="headcule_list" id="searchbutton" value="查  询"
			timeout="2500" 
		...
		<sj:grid id="gridtable" caption="总行或兄弟行来源列表" dataType="json"
			href="../headcule/headcule_list.shtml" pager="true" gridModel="gridModel"
			rowList="10,15,20" rowNum="20" rownumbers="true" viewrecords="true"
			multiselect="true"
			cssStyle="line-height:30px;" onSelectRowTopics="rowselect"
			height="400">
			<sj:gridColumn name="hosid" index="hosid" title="序号" sortable="true" width="100" search="true" searchoptions="{sopt:['eq','ne','lt','gt']}" />
			...
			<sj:gridColumn name="idnumber" index="idnumber" title="证件号" sortable="false"
				width="110" />
			<sj:gridColumn name="hid" index="hid" title="案件总编号" sortable="true"
				width="150" />
			...
			<sj:gridColumn name="sponsor" index="sponsor" title="来函单位主办人" sortable="false"
				width="160" />
			<sj:gridColumn name="hosid" index="hsid" title="hosid" sortable="false" hidden="true"
				width="110" />
			<%-- 
