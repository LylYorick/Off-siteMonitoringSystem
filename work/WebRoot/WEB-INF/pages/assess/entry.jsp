	    ...
	    $.subscribe('errorState', function(event,data) { 
	  	...
	  	$.subscribe('add', function(event,data) { 
    		...
    		var r = $("#gridtable").jqGrid('getRowData',s);
	  		window.location.href="<%=request.getContextPath()%>/assess/assess_add.shtml?oid="+r.oid+"&year="+r.xxyear+"&selfevalflag=a";
	    });
	  	$.subscribe('view', function(event,data) { 
    		...
    		var r = $("#gridtable").jqGrid('getRowData',s);
	  		window.location.href="<%=request.getContextPath()%>/assess/assess_view.shtml?oid="+r.oid+"&year="+r.xxyear+"&selfevalflag=a";
	    });
	  	$.subscribe('view2', function(event,data) { 
	       	var s = $("#gridtable").jqGrid('getGridParam','selrow');
	       	if(s==null){
   	        alert("请选择记录！");
   	        return;
    		...
    		var r = $("#gridtable").jqGrid('getRowData',s);
	  		window.location.href="<%=request.getContextPath()%>/assess/assess_view2.shtml?oid="+r.oid+"&year="+r.xxyear+"&selfevalflag=b";
	    });
	     	$(getsort);   
function getsort(){   
    var cid=$("#bpid").val();   
...
<body>
	...
	<fieldset>
		...
		<sj:submit id="grid_view_colsbutton2" value="自 评 明 细"
			onClickTopics="view2" button="true" />
		<sj:grid id="gridtable" caption="金融机构评分列表" dataType="json"
			href="../assess/assess_list.shtml" pager="true"
			...
			<sj:gridColumn name="bname" index="bname" title="银行名称"
				sortable="false" width="402" />
			<sj:gridColumn name="xxyear" index="xxyear" title="年度" sortable="false"
				width="116" />
			<sj:gridColumn name="selfevalscore" index="selfevalscore" title="自评分数"
				formatter="checkNum" sortable="false" width="116" />
