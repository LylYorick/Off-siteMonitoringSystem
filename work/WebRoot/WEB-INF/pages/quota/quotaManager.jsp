...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/common.js"></script>
	...
	$.subscribe('upload', function(event,data) {
	    	...
	    	document.location.href="<%=request.getContextPath()%>/quota/quota_quotaAdd.shtml";
    	});
	...
	$.subscribe('downloadfile', function(event,data) {
    	...
    	document.location.href="<%=request.getContextPath()%>/quota/quota_download.shtml?oids="+oid+"&ins_ids="+ins_id; 
    	}); 
     ...
     $.subscribe('deletefile', function(event,data) {
	    ...
	    <%-- 	document.location.href="<%=request.getContextPath()%>/quota/quota_delete.shtml?ins_ids="+ins_id; --%>
		    	$.ajax({
		            cache:false,   
		            url:'<%=request.getContextPath()%>/quota/quota_delete.shtml',    
                    data:{ins_ids:ins_id},
		            type:'post',   
		            dataType:'json',   
...
<body>
	<div class="grid">
		<fieldset>
			...
			<br>
			<s:form namespace="/quota" action="quota_list" method="post">

				<s:bean name="java.util.HashMap" id="qTableLayout">
					<s:param name="tablecolspan" value="%{4}" />
				</s:bean>
				<s:textfield label="年度" name="year">
					<s:param name="labelcolspan" value="%{1}" />
					<s:param name="inputcolspan" value="%{3}" />
				</s:textfield>
				<s:select list="#list" label="评级表类型" id="bpid" name="bid" listKey="id.bfirstid" listValue="firstCatname" onchange="getsort()">
					<s:param name="labelcolspan" value="%{1}" />
					<s:param name="inputcolspan" value="%{1}" />
				</s:select>
				<s:select list="#{'9999':'--请选择--'}" label="一级指标名称" id="bannerid" name="oid" cssStyle="width:240px">
					<s:param name="labelcolspan" value="%{1}" />
					<s:param name="inputcolspan" value="%{1}" />
				</s:select>
				<s:select list="#{'9999':'--请选择--'}" label="二级指标名称" id="bannerid" name="oid" cssStyle="width:240px">
					<s:param name="labelcolspan" value="%{1}" />
					<s:param name="inputcolspan" value="%{1}" />
				</s:select>
			</s:form>
			<div align="center">
				<sj:submit formIds="quota_list" id="searchbutton" value="查  询"
		...
		<sj:submit id="grid_upload_colsbutton" value="保存修改"
			onClickTopics="upload" button="true"/>
		<sj:submit id="grid_download_colsbutton" value="删除" timeout="2500"
			onClickTopics="downloadfile" button="true" />
		<sj:submit id="grid_delete_colsbutton" value="新增指标" timeout="2500"
			onClickTopics="deletefile" button="true" indicator="loading" loadingText="正在删除..."/>
			
		<sj:grid id="gridtable" caption="指标列表" dataType="json"
			href="../quota/quota_list.shtml" pager="true" gridModel="gridModel"
			...
			<sj:gridColumn name="id" index="id" title="指标ID" sortable="true" hidden="true" width="70"/>
