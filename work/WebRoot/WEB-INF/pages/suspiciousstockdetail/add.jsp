...
<head>
	<title>重点可疑交易报告管理>>证券业可疑交易明细信息录入</title>
</head>
<body>
<script type="text/javascript"> 
	...
	<div class="grid">
		<fieldset>
		<legend>
			证券业可疑交易明细信息录入<input id="sj" type="hidden" />
		</legend>
		<br>
		<s:form namespace="/suspiciousstockdetail" action="suspiciousstockdetail_save" method="post"  enctype="multipart/form-data">
			<s:bean name="java.util.HashMap" id="qTableLayout">
				<s:param name="tablecolspan" value="%{6}" />
			</s:bean>
			
			<s:hidden name="BStockBase.sid" value="%{#id}"></s:hidden>
            <s:textfield label="金融机构名称" required="true" name="organname" value="%{#info.bname}" readonly="true">
				<s:param name="labelcolspan" value="%{1}" />
				<s:param name="inputcolspan" value="%{2}" />
			</s:textfield>
			<s:textfield label="资金/结算账户开户行名称" required="true" name="bname">
				<s:param name="labelcolspan" value="%{1}" />
				<s:param name="inputcolspan" value="%{2}" />
			</s:textfield>			
