...
<head>
	<title>重点可疑交易报告管理>>保险业可疑交易明细信息修改</title>
</head>
<body>
	...
	<div class="grid">
		<fieldset>
		<legend>
			保险业可疑交易明细信息修改<input id="sj" type="hidden" />
		</legend>
		<br>
		<s:form namespace="/suspiciousinsurancedetail" action="suspiciousinsurancedetail_update" method="post"  enctype="multipart/form-data">
			<s:bean name="java.util.HashMap" id="qTableLayout">
				<s:param name="tablecolspan" value="%{6}" />
			</s:bean>
			<s:property value="#i#info.dssid" />
			<s:hidden name="dssid" value="%{#info.dssid}"></s:hidden>
			<s:hidden name="BInsuranceBase.siid" value="%{#info.BInsuranceBase.siid}"></s:hidden>			
			...
			<s:textfield label="资金账户开户行" required="true" name="bname" value="%{#info.bname}">
				<s:param name="labelcolspan" value="%{1}" />
				<s:param name="inputcolspan" value="%{2}" />
			</s:textfield>			
