...
<script type="text/javascript"> 
    ...
    $.subscribe('add', function(event,data) {
    	...
    	document.location.href="<%=request.getContextPath()%>/suspiciousstock/suspiciousstock_add.shtml";
    	});  
    $.subscribe('stockview', function(event,data) { 
    		...
    		//alert(r.sid);		 		
	  		window.location.href="<%=request.getContextPath()%>/suspiciousstock/suspiciousstock_stockview.shtml?id="+r.sid;
	    });    	  
    ...
    $.subscribe('createxls', function(event,data) {
    	...
    	//alert("cname="+cname+" lineid="+lineid+" starttime="+starttime+" endtime="+endtime+" ccid="+ccid);
	  	window.location.href="<%=request.getContextPath()%>/suspiciousstock/suspiciousstock_downloadstock.shtml?ccid="+ccid+"&cname="+cname+"&starttime="+starttime+"&endtime="+endtime+"&lineid="+lineid;
  	});	
    </script>     
	<div class="grid">
		<fieldset>
		...
		<br>
		<s:form namespace="/suspiciousstock" action="suspiciousstock_stocklist" method="post">
		...
		<div align="center">
			<sj:submit formIds="suspiciousstock_stocklist" id="searchbutton" value="查  询"
			timeout="2500" 
     ...
     <s:if test="#session.userinfo.information.bname != ''">
		...
		<sj:submit id="grid_stockview_colsbutton" value="线索信息查询"
			onClickTopics="stockview" button="true"/>
	
		<sj:grid id="gridtable" caption="重点可疑交易报告列表" dataType="json"
			href="../suspiciousstock/suspiciousstock_stocklist.shtml" pager="true" gridModel="gridModel"
			rowList="10,15,20" rowNum="20" rownumbers="true" viewrecords="true"
			multiselect="true" 
			cssStyle="line-height:30px;" onSelectRowTopics="rowselect"
			height="400">
			<sj:gridColumn name="sid" index="sid" title="ID" sortable="false" width="50" hidden="true"/>
