...
<head>
	<title>反洗钱调查信息管理>>线索来源(举报或专报)查询</title>
</head>
<body>
<script type="text/javascript">
    ...
    $.subscribe('errorState', function(event,data) { 
 	...
 	$.subscribe('add', function(event,data) {
   	var s; 
   	s = $("#gridtable").jqGrid('getGridParam','selarrrow');
   	document.location.href="<%=request.getContextPath()%>/reportcule/reportcule_add.shtml";
   	}); 
   	$.subscribe('modify', function(event,data) {
   	var s = $("#gridtable").jqGrid('getGridParam','selarrrow');
   	var r = $("#gridtable").jqGrid('getRowData',s);
   	document.location.href="<%=request.getContextPath()%>/reportcule/reportcule_modify.shtml?rsid="+r.rsid;
   	});
   	
   	$.subscribe('delete', function(event,data) {
   	var s = $("#gridtable").jqGrid('getGridParam','selarrrow');
   	var r = $("#gridtable").jqGrid('getRowData',s);
   	document.location.href="<%=request.getContextPath()%>/reportcule/reportcule_delete.shtml?rsid="+r.rsid;
   	});
    </script>     
	<div class="grid">
		<fieldset>
		...
		<br>
		<s:form namespace="/reportcule" action="reportcule_list" method="post">

			<s:bean name="java.util.HashMap" id="qTableLayout">
				<s:param name="tablecolspan" value="%{4}" />
			</s:bean>
			<s:textfield label="案件总编号" name="rid" id="rid"></s:textfield>
			<s:textfield label="调查名称" name="researchname" id="researchname"></s:textfield>
		...
		<div align="center">
			<sj:submit formIds="reportcule_list" id="searchbutton" value="查  询"
			timeout="2500" 
		...
		<sj:grid id="gridtable" caption="举报或专报信息列表" dataType="json"
			href="../reportcule/reportcule_list.shtml" pager="true" gridModel="gridModel"
			rowList="10,15,20" rowNum="20" rownumbers="true" viewrecords="true"
			multiselect="true" 
			cssStyle="line-height:30px;" onSelectRowTopics="rowselect"
			height="400">
			<sj:gridColumn name="rsid" index="rsid" title="序号" sortable="true"
			...
			<sj:gridColumn name="idnumber" index="idnumber" title="证件号" sortable="false"
				width="110" />
			<sj:gridColumn name="rid" index="rid" title="案件总编号" sortable="false"
				width="150" />
			<sj:gridColumn name="researchname" index="researchname" title="调查名称" sortable="false"
				width="100" />
